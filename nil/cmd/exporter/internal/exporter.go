...
func (e *exporter) runTopFetcher(ctx context.Context, shardId types.ShardId, from types.BlockNumber) error {
	...
	for {
		...
		case <-ticker.C:
			...
			logger.Info().Msgf("Fetching blocks from %d to %d", from, topBlock.Id)
			from, err = e.pushBlocks(ctx, shardId, from, topBlock.Id)
			if err != nil {
				logger.Error().Err(err).Msg("Failed to fetch blocks")
				continue
			}
			check.PanicIfNot(from == topBlock.Id)
			e.blocksChan <- &BlockWithShardId{topBlock, shardId}
			from++
...
func (e *exporter) runBottomFetcher(ctx context.Context, shardId types.ShardId, to types.BlockNumber) error {
	...
	for from < to {
		...
		case <-ticker.C:
			...
			nextPresentId, err := e.driver.FetchNextPresentBlockId(ctx, shardId, from)
			if err != nil {
				logger.Error().Err(err).Msg("Failed to fetch next present block.")
				continue
			}
			check.PanicIfNot(nextPresentId <= to)

			logger.Info().Msgf("Fetching blocks from %d to %d", from, nextPresentId)
			from, err = e.pushBlocks(ctx, shardId, from, nextPresentId)
			if err != nil {
				logger.Error().Err(err).Msg("Failed to fetch blocks")
				continue
			}
